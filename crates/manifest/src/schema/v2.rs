...
impl AppManifest {
    ...
    pub fn validate_depdendencies(&self) -> anyhow::Result<()> {
...
impl ComponentDependencies {
    ...
    fn check_disjoint(this: &DependencyName, other: &DependencyName) -> anyhow::Result<()> {
        ...
        if let (Some(this), Some(other)) = (this.version.clone(), other.version.clone()) {
            if Self::normalize_version(this) != Self::normalize_version(other) {
                return Ok(());
            }
        }

        if let (Some(this), Some(other)) = (this.interface.as_ref(), other.interface.as_ref()) {
            if this != other {
                return Ok(());
            }
        }
    ...
    fn normalize_version(mut version: semver::Version) -> semver::Version {
