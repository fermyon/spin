...
/// Generic application configuration used throughout Spin.
...
pub struct Config<T: Default> {
    ...
    pub components: Vec<ComponentConfig<T>>,
}
...
#[derive(Clone, Debug, Default, Deserialize, Serialize)]
#[serde(deny_unknown_fields, rename_all = "camelCase")]
pub struct ComponentConfig<T: Default> {
    /// The module source.
...
/// Configuration for a local component.
/// Used to either run the application directly with Spin,
/// or to construct a bindle and push it to the registry.
#[derive(Clone, Debug, Default, Deserialize, Serialize)]
#[serde(deny_unknown_fields, rename_all = "camelCase")]
...
/// Configuration for a linked component.
/// This should be generated by a Spin linker, and is the
/// input for  
#[derive(Clone, Debug, Default, Deserialize, Serialize)]
#[serde(deny_unknown_fields, rename_all = "camelCase")]
pub struct LinkedComponentConfig {}

/// Component configuration that is shared across multiple
...
pub enum ModuleSource {
    /// Local path to a module that needs to be linked.
    Local(PathBuf),
...
mod tests {
    ...
    fn test_local_config() -> Result<()> {
        let cfg: Config<LocalComponentConfig> = toml::from_str(CFG_TEST)?;
        println!("{:#?}", cfg);
        ...
        let b = match cfg.components[1].source.clone() {
            ...
            ModuleSource::Local(_) => panic!("expected bindle source"),
