...
use wasmtime_wasi::{TrappableError, WasiImpl, WasiView};
...
impl UdpSocket {
    async fn finish_connect<T: WasiView>(
        ...
        let result = convert_result(
            <WasiImpl<&mut T> as latest::sockets::udp::HostUdpSocket>::stream(
                &mut WasiImpl(table),
                borrow,
                addr.map(|a| a.into()),
            )
    ...
    T: WasiView,
{
    ...
    async fn send(
        ...
        match convert_result(
            <Self as latest::sockets::udp::HostOutgoingDatagramStream>::check_send(self, outgoing2),
        )? {
            Ok(n) => {
    ...
    fn local_address(
        ...
        let socket = self.table().get(&self_)?.inner()?;
        convert_result(<Self as latest::sockets::udp::HostUdpSocket>::local_address(self, socket))
    }
    ...
    fn remote_address(
        ...
        let socket = self.table().get(&self_)?.inner()?;
        convert_result(<Self as latest::sockets::udp::HostUdpSocket>::remote_address(self, socket))
    }

    fn address_family(&mut self, self_: Resource<UdpSocket>) -> wasmtime::Result<IpAddressFamily> {
        let socket = self.table().get(&self_)?.inner()?;
        <Self as latest::sockets::udp::HostUdpSocket>::address_family(self, socket)
            .map(|e| e.into())
    }
    ...
    fn unicast_hop_limit(
        ...
        let socket = self.table().get(&self_)?.inner()?;
        convert_result(
            <Self as latest::sockets::udp::HostUdpSocket>::unicast_hop_limit(self, socket),
        )
    }
    ...
    fn receive_buffer_size(
        ...
        let socket = self.table().get(&self_)?.inner()?;
        convert_result(
            <Self as latest::sockets::udp::HostUdpSocket>::receive_buffer_size(self, socket),
        )
    }
    ...
    fn send_buffer_size(
        ...
        let socket = self.table().get(&self_)?.inner()?;
        convert_result(
            <Self as latest::sockets::udp::HostUdpSocket>::send_buffer_size(self, socket),
        )
    }
    ...
    fn set_send_buffer_size(
        ...
        let socket = self.table().get(&self_)?.inner()?;
        convert_result(
            <Self as latest::sockets::udp::HostUdpSocket>::set_send_buffer_size(
                self, socket, value,
            ),
        )
    }
    ...
    fn subscribe(&mut self, self_: Resource<UdpSocket>) -> wasmtime::Result<Resource<Pollable>> {
        let socket = self.table().get(&self_)?.inner()?;
        <Self as latest::sockets::udp::HostUdpSocket>::subscribe(self, socket)
    }
    ...
    fn drop(&mut self, rep: Resource<UdpSocket>) -> wasmtime::Result<()> {
        ...
        let socket = match me {
            ...
            UdpSocket::Connected {
                ...
                <Self as latest::sockets::udp::HostIncomingDatagramStream>::drop(self, incoming)?;
                <Self as latest::sockets::udp::HostOutgoingDatagramStream>::drop(self, outgoing)?;
                socket
            }
            UdpSocket::Dummy => return Ok(()),
        };
        <Self as latest::sockets::udp::HostUdpSocket>::drop(self, socket)
    }
}
    ...
    T: WasiHttpView + Send,
{
    fn new(
        ...
        match <Self as latest::http::types::HostFields>::from_list(self, entries)? {
            Ok(fields) => Ok(fields),
            Err(e) => Err(e.into()),
        }
    }
    ...
    T: WasiHttpView + Send,
{
    fn new(
        ...
        let request = <Self as latest::http::types::HostOutgoingRequest>::new(self, headers)?;
        let borrow = || Resource::new_borrow(request.rep());

        if let Err(()) = <Self as latest::http::types::HostOutgoingRequest>::set_method(
            self,
            borrow(),
            method.into(),
        )? {
            <Self as latest::http::types::HostOutgoingRequest>::drop(self, request)?;
            anyhow::bail!("invalid method supplied");
        }

        if let Err(()) = <Self as latest::http::types::HostOutgoingRequest>::set_path_with_query(
            self,
            borrow(),
            path_with_query,
        )? {
        ...
        if let Err(()) = <Self as latest::http::types::HostOutgoingRequest>::set_scheme(
            self,
            borrow(),
            scheme.map(|s| s.into()),
        )? {
        ...
        if let Err(()) = <Self as latest::http::types::HostOutgoingRequest>::set_authority(
            self,
            borrow(),
            Some(authority),
        )? {
    ...
    T: WasiHttpView + Send,
{
    fn new(
        ...
        let headers = <Self as latest::http::types::HostFields>::clone(self, headers)?;
        let response = <Self as latest::http::types::HostOutgoingResponse>::new(self, headers)?;
        let borrow = || Resource::new_borrow(response.rep());

        if let Err(()) = <Self as latest::http::types::HostOutgoingResponse>::set_status_code(
            self,
            borrow(),
            status_code,
        )? {
            <Self as latest::http::types::HostOutgoingResponse>::drop(self, response)?;
    ...
    T: WasiHttpView + Send,
{
    ...
    fn finish(
        ...
        <Self as latest::http::types::HostOutgoingBody>::finish(self, this, trailers)?;
        Ok(())
    }
    ...
    T: WasiHttpView + Send,
{
    fn set(
        ...
        <Self as latest::http::types::HostResponseOutparam>::set(self, param, response)
    }
    ...
    T: WasiHttpView + Send,
{
    ...
    fn get(
        ...
        match <Self as latest::http::types::HostFutureTrailers>::get(self, self_)? {
            ...
            Some(Ok(Ok(None))) => Ok(Some(Ok(<Self as latest::http::types::HostFields>::new(
                self,
            )?))),
    ...
    ) -> wasmtime::Result<Result<wasmtime::component::Resource<FutureIncomingResponse>, HttpError>>
    {
        let options = match options {
            Some(RequestOptions {
                ...
                let options = <Self as latest::http::types::HostRequestOptions>::new(self)?;
                let borrow = || Resource::new_borrow(request.rep());
                ...
                if let Some(ms) = connect_timeout_ms {
                    ...
                    {
                        <Self as latest::http::types::HostRequestOptions>::drop(self, options)?;
