...
pub fn validate_webhook(req: Request, callback_allowed: bool, rate: &str) -> Result<Response> {
    ...
    if callback_allowed {
        let callback = req.headers().iter().find(|h| h.0 == CALLBACK_REQUEST_HEADER);
        ...
        res = send_request(req).map_err(|err| {
            anyhow::anyhow!("failed to send request with error: {:?}", err)
        })?;
    }
    res.headers_mut().insert(ORIGIN_RESPONSE_HEADER, origin.into());
    res.headers_mut().insert(RATE_RESPONSE_HEADER, rate.try_into()?);
    Ok(res)
}
...
fn validate_rate(rate: &str) -> Result<&str> {
    if rate != "*" {
        match i32::from_str(rate) {
            Ok(rate) => {
                if rate < 1 {
                    return Err(anyhow::anyhow!("invalid rate: {}, not a positive number", rate));
                }
            }
            Err(_) => {
                return Err(anyhow::anyhow!("invalid rate: {}, not a positive number or *", rate));
            }
        }
    };
    Ok(rate)
...
mod tests {
    ...
    fn test_validate_webhook() -> Result<()> {
        ...
        // assert_eq!(res.headers().get(ORIGIN_RESPONSE_HEADER).and_then(|v| Some(v.to_str().unwrap())), Some("eventemitter.example.com"));
        // assert_eq!(res.headers().get(RATE_RESPONSE_HEADER).and_then(|v| Some(v.to_str().unwrap())), Some("*"));
        Ok(())
    }
}