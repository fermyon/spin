...
pub async fn compose<'a, L: ComponentLoader>(
    loader: &'a L,
...
impl<'a, L: ComponentLoader> Composer<'a, L> {
    ...
    async fn prepare_dependencies(
        ...
        for (dependency_name, dependency) in component.dependencies.iter() {
            let mut matched = Vec::new();

            for import_name in import_keys.iter() {
            ...
            for import_name in matched.into_iter() {
                mappings
