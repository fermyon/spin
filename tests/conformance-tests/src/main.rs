use conformance_tests::config::KeyValueStorePrecondition;
...
fn main() {
    ...
    for test in conformance_tests::tests(&tests_dir).unwrap() {
        ...
        for precondition in &test.config.preconditions {
            match precondition {
                conformance_tests::config::Precondition::KeyValueStore(
                    KeyValueStorePrecondition { label },
                ) => {
                    if label != "default" {
                        panic!("Unsupported precondition: KeyValueStore with label '{label}'")
                    }
                }
