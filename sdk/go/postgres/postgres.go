...
func lowerParameterValues(params []ParameterValue) C.outbound_pg_list_parameter_value_t {
	...
	if len(params) == 0 {
		...
		for lower_params_i := range params {
			...
			// maybe unnecessary marshal because of component model?
...
func liftPgError(err *C.outbound_pg_pg_error_t) PgError {
	var lift_err PgError
	var lift_err_val PgError
	if err.tag == 0 {
		lift_err_val = PgErrorSuccess()
	}
	if err.tag == 1 {
		lift_err_val_ptr := *(*C.outbound_pg_string_t)(unsafe.Pointer(&err.val))
		lift_err_val_val := C.GoStringN(lift_err_val_ptr.ptr, C.int(lift_err_val_ptr.len))
		lift_err_val = PgErrorConnectionFailed(lift_err_val_val)
	}
	if err.tag == 2 {
		lift_err_val_ptr := *(*C.outbound_pg_string_t)(unsafe.Pointer(&err.val))
		lift_err_val_val := C.GoStringN(lift_err_val_ptr.ptr, C.int(lift_err_val_ptr.len))
		lift_err_val = PgErrorBadParameter(lift_err_val_val)
	}
	if err.tag == 3 {
		lift_err_val_ptr := *(*C.outbound_pg_string_t)(unsafe.Pointer(&err.val))
		lift_err_val_val := C.GoStringN(lift_err_val_ptr.ptr, C.int(lift_err_val_ptr.len))
		lift_err_val = PgErrorQueryFailed(lift_err_val_val)
	}
	if err.tag == 4 {
		lift_err_val_ptr := *(*C.outbound_pg_string_t)(unsafe.Pointer(&err.val))
		lift_err_val_val := C.GoStringN(lift_err_val_ptr.ptr, C.int(lift_err_val_ptr.len))
		lift_err_val = PgErrorValueConversionFailed(lift_err_val_val)
	}
	if err.tag == 5 {
		lift_err_val_ptr := *(*C.outbound_pg_string_t)(unsafe.Pointer(&err.val))
		lift_err_val_val := C.GoStringN(lift_err_val_ptr.ptr, C.int(lift_err_val_ptr.len))
		lift_err_val = PgErrorOtherError(lift_err_val_val)
	}
	lift_err = lift_err_val
	return lift_err
}
...
func Query(address string, statement string, params []ParameterValue) (RowSet, PgError) {
	...
	if result.is_err {
		...
		return rowset, PgError{}
	}
}

func Execute(address string, statement string, params []ParameterValue) (uint64, PgError) {
	lower_address := lowerPgStr(address)
	defer C.outbound_pg_string_free(&lower_address)
