...
// Benchmark SpinHttpExecutor time to start and process one request
fn bench_spin_startup(c: &mut Criterion) {
    let async_runtime = Runtime::new().unwrap();
    c.bench_function("spin/startup", |b| {
...
fn bench_spin_concurrency_minimal(c: &mut Criterion) {
    ...
    for concurrency in concurrency_steps() {
        ...
        group.bench_function(
            format!("concurrency-{}/sleep-{}ms", concurrency, sleep_ms),
            |b| {
...
fn bench_wagi_concurrency_minimal(c: &mut Criterion) {
    ...
    let mut group = c.benchmark_group("wagi");
    for concurrency in concurrency_steps() {
        let sleep_ms = 1;
        let bench_inner = || {
            black_box(run_concurrent_requests(
